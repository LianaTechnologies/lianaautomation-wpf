<?php
/**
 * Admin Notices for LianaAutomation for WPForms.
 *
 * @package LianaAutomation for WPForms
 */

namespace LianaAutomation_WPF;
...
class Admin_Notices {
	...
	public function register_deprecated_plugin( array $plugins ): array {
		$plugins['lianaautomation-wpf'] = __( 'LianaAutomation for WPForms', 'lianaautomation-wpf' );
		return $plugins;
	}
		...
		<div class="notice notice-warning liana-deprecation-notice is-dismissible" data-nonce="<?php echo \esc_attr( $nonce ); ?>">
			...
			<div class="liana-notice-content">
				<?php if ( $has_single_plugin ) : ?>
					<p>
						<strong><?php \esc_html_e( 'Deprecation notice!', 'lianaautomation-wpf' ); ?></strong>
						<?php
						printf(
							/* translators: 1: plugin name, 2: opening bold tag, 3: closing bold tag, 4: opening link tag, 5: closing link tag */
							\__( "%1\$s's functionality has now been included in %2\$sLiana with GrowthStack%3\$s. Please install %4\$sLiana with GrowthStack%5\$s in order to keep receiving updates.", 'lianaautomation-wpf' ),
				...
				<?php else : ?>
					<p>
						...
						printf(
							...
							\__( 'The following Liana plugins have been deprecated and their functionality is now included in %1$sLiana with GrowthStack%2$s. Please install %3$sLiana with GrowthStack%4$s to keep receiving updates:', 'lianaautomation-wpf' ),
							'<strong>',
							'</strong>',
							'<a href="' . \esc_url( $growthstack_url ) . '" target="_blank" rel="noopener">',
							'</a>'
